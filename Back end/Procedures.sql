...
ELSE IF @userType = 'Lecturers' BEGIN
    ...
    VALUES
        (@user_id, NULL)
END
ELSE IF @userType = 'Teaching assistants' BEGIN
    INSERT INTO Milestone2.dbo.TeachingAssistant
        (TAID, schedule)
    VALUES
        (@user_id, NULL)
END
ELSE IF @userType = 'External examiners' BEGIN
...
IF @User_id = NULL BEGIN
    ...
    END
    ELSE IF @User_type = 'Teaching assistants' BEGIN
        SELECT *
        FROM TeachingAssistant
            INNER JOIN Users ON TeachingAssistant.TAID = Users.userID
    END
    ELSE IF @User_type = 'External examiners' BEGIN
        SELECT *
...
@tempOutput2 OUTPUT,
@userType = 'Lecturers',
@userName = 'Nadaorsmthidk',
@email = 'nada.abdhelnadder@giu-uni.de',
@first_name = NULL,
@last_name = NULL,
@birth_date = NULL,
@GPA = NULL,
